...
get_results(Account, View, ViewOptions, _Reason, Retry) ->
    ...
    EncodedMODb = kz_term:format_account_modb(AccountMODb, 'encoded'),
...
get_modb(?MATCH_MODB_SUFFIX_ENCODED(_,_,_) = AccountMODb) ->
    kz_term:format_account_modb(AccountMODb, 'raw');
...
get_modb(Account, Timestamp) ->
    kz_term:format_account_mod_id(Account, Timestamp).
...
get_modb(Account, Year, Month) ->
    AccountDb = kz_term:format_account_id(Account, 'encoded'),
    kz_term:format_account_mod_id(AccountDb, Year, Month).
...
maybe_create_current_modb(?MATCH_MODB_SUFFIX_ENCODED(_, _, _) = AccountMODb, Options) ->
    maybe_create_current_modb(kz_term:format_account_modb(AccountMODb, 'raw'), Options);
...
maybe_create(?MATCH_MODB_SUFFIX_ENCODED(_, _, _) = AccountMODb) ->
    maybe_create(kz_term:format_account_modb(AccountMODb, 'raw'));
...
%% AccountIds should be kzd_account:get_all_accounts('raw').
...
maybe_delete(AccountMODb, AccountIds) ->
    AccountId = kz_term:format_account_id(AccountMODb, 'raw'),
