...
timezone(JObj, Default) ->
    case kz_json:get_value(?KEY_TIMEZONE, JObj, Default) of
        'undefined' -> kzd_account:timezone(kz_doc:account_id(JObj));
        <<"inherit">> -> kzd_account:timezone(kz_doc:account_id(JObj)); %% UI-1808
        TZ -> TZ
    end.
...
fax_settings(JObj) ->
    ...
    AccountFaxSettings = kzd_account:fax_settings(kz_doc:account_id(JObj)),
