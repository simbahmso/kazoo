...
find_webhooks(HookEvent, AccountId) ->
    case kzd_account:fetch(AccountId, 'accounts') of
        {'ok', JObj} ->
            Accounts = kzd_account:tree(JObj) -- [AccountId],
...
save_attempt(AccountId, Attempt) ->
    ...
    ModDb = kzd_account:format_account_mod_id(AccountId, Now),
...
init_webhook(Acct, Year, Month) ->
    Db = kzd_account:format_account_id(kz_json:get_value(<<"key">>, Acct), Year, Month),
