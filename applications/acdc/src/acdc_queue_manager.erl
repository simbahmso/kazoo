...
-define(BINDINGS(A, Q), [{'conf', [{'type', <<"queue">>}
                                  ,{'db', kz_term:format_account_id(A, 'encoded')}
                                  ,{'id', Q}
                                  ,'federate'
                                  ]}
...
init([Super, AccountId, QueueId]) ->
    ...
    AcctDb = kz_term:format_account_id(AccountId, 'encoded'),
...
handle_cast({'start_workers'}, #state{account_id=AccountId
    ...
    case kz_datamgr:get_results(kz_term:format_account_id(AccountId, 'encoded')
