...
filter_messages([Message|Messages], <<_/binary>> = Filter, Context, Selected)
  when Filter =:= ?VM_FOLDER_NEW;
    ...
    Id = kzd_box_message:media_id(Message),
    QsFiltered = filtered_by_qs(Message, crossbar_doc:has_qs_filter(Context), Context),
    case QsFiltered
...
create_new_message_document(Context, BoxJObj) ->
    ...
    AccountRealm = case kzd_account:fetch_realm(AccountId) of 'undefined' -> <<"nodomain">>; R -> R end,
