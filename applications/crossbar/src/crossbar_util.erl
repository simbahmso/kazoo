...
flush_registrations(Context) ->
    flush_registrations(kzd_account:fetch_realm(cb_context:account_id(Context))).
...
flush_registration(Username, <<_/binary>> = Realm) ->
    ...
    kz_amqp_worker:cast(FlushCmd, fun kapi_switch:publish_check_sync/1),
    kz_amqp_worker:cast(FlushCmd, fun kapi_registration:publish_flush/1);
...
update_descendants_tree([Descendant|Descendants], Tree, NewResellerId, MovedAccountId) ->
    case kzd_account:fetch(Descendant) of
        {'error', _E}=Error -> Error;
        {'ok', AccountJObj} ->
            PreviousTree = kzd_account:tree(AccountJObj),
            ...
            AccountDb = kz_term:format_account_db(Descendant),
            case kz_datamgr:save_doc(AccountDb, kz_json:set_values(Values, AccountJObj)) of
                ...
                {'ok', NewAccountJObj} ->
                    ...
                    AccountId = kz_term:format_account_id(Descendant),
    ...
    case kz_services:fetch_services_doc(AccountId, true) of
        ...
        {'ok', JObj} ->
            Props = props:filter_undefined(
                      ...
                      ,{?SERVICES_PVT_TREE_PREVIOUSLY, kzd_account:tree(JObj)}
...
get_tree(<<_/binary>> = Account) ->
    case kzd_account:fetch(Account) of
...
change_pvt_enabled(State, AccountId) ->
    ...
    try
        ...
        JObj2 = case State of
                    true -> kzd_account:enable(JObj1);
                    false -> kzd_account:disable(JObj1)
                end,
...
get_account_lang(AccountId) ->
    case kzd_account:fetch(AccountId) of
        {'ok', AccountJObj} ->
            case kzd_account:language(AccountJObj) of
                undefined -> error;
                Lang -> {'ok', Lang}
            end;
        {'error', _E} ->
...
get_user_timezone(AccountId, UserId) ->
    case kzd_user:fetch(AccountId, UserId) of
        ...
        {'error', _E} -> kzd_account:timezone(AccountId)
    end.
...
create_auth_token(Context, AuthModule, JObj) ->
    ...
    JObjToken = kz_doc:update_pvt_parameters(kz_json:from_list(Token)
                                            ,kz_term:format_account_id(AccountId, 'encoded')
                                            ,Token
                                            ),
...
maybe_refresh_fs_xml('user', Context, 'true') ->
    ...
    Realm = kzd_account:fetch_realm(AccountDb),
    Id = kz_doc:id(Doc),
...
maybe_refresh_fs_xml('device', Context, Precondition) ->
    ...
    ( Precondition
      ...
      or (kz_json:is_true(<<"enabled">>, DbDoc)
        ...
        andalso refresh_fs_xml(kzd_account:fetch_realm(cb_context:account_db(Context))
                              ,DbDoc
                              ),
    'ok';
maybe_refresh_fs_xml('account', Context, _) ->
    Devices = get_account_devices(cb_context:account_id(Context)),
    Realm = kzd_account:fetch_realm(cb_context:account_db(Context)),
    lists:foreach(fun(DevDoc) -> refresh_fs_xml(Realm, DevDoc) end, Devices);
